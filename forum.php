...
<div class="container mt-3 pt-2">
   ...
   <?php
    $event = $conn1->query("SELECT f.*,u.name from forum_topics f inner join users u on u.id = f.user_id order by f.id desc");
    while($row = $event->fetch_assoc()):
        ...
        $count_comments=0;
        $count_comments = $conn1->query("SELECT * FROM forum_comments where topic_id = ".$row['id'])->num_rows;
    ?>
